...
import com.jtljia.pump.canal.protocol.CanalEntry.Header;
import com.jtljia.pump.canal.protocol.CanalEntry.Pair;
import com.jtljia.pump.canal.protocol.CanalEntry.RowChange;
import com.jtljia.pump.canal.protocol.CanalEntry.RowData;
import com.jtljia.pump.canal.protocol.CanalEntry.TransactionBegin;
import com.jtljia.pump.canal.protocol.CanalEntry.TransactionEnd;
import com.jtljia.pump.canal.protocol.CanalEntry.Type;
 ...
 * @version 1.0.0
 */
public class MySQLBinLogParser implements BinlogParser<LogEvent> {
    ...
    public static final Logger          logger              = LoggerFactory.getLogger(MySQLBinLogParser.class);
    ...
    public Entry parse(LogEvent logEvent) throws ParseException {
    ...
    private Entry parseRowsEvent(RowsLogEvent event) {
        ...
        if (table == null) {            // tableId对应的记录不存在
            throw new TableNotFoundException("not found table for table id:" + event.getTableId());
        }
        ...
        try {
            ...
            */
            ...
            if (LogEvent.WRITE_ROWS_EVENT_V1 == type || LogEvent.WRITE_ROWS_EVENT == type) {
                ...
                throw new ParseException("unsupport event type :" + event.getHeader().getType());
            }
    ...
    private boolean parseOneRow(RowData.Builder rowDataBuilder, RowsLogEvent event, RowsLogBuffer buffer, BitSet cols,
        ...
        if (tableMeta != null && columnInfo.length > tableMeta.getFileds().size()) {
            ...
            if (tableMeta == null) {
                tableError = true;
                if (!filterTableError) {
                    throw new ParseException("not found [" + event.getTable().getDbName() + "."
            ...
            if (tableMeta != null && columnInfo.length > tableMeta.getFileds().size()) {
                tableError = true;
                if (!filterTableError) {
                    throw new ParseException("column size is not match for table:" + tableMeta.getFullName() + ","
